...
export function setupInputHandlers(canvas, getSession, { DeviceEvent, InputTransaction }, isInputSuppressed) {
    const handleKeyDown = (e) => {
        // If modal or settings is open, don't pass keys to remote RDP
        if (isInputSuppressed && isInputSuppressed()) return;
    ...
    // Return cleanup function to prevent memory leaks and duplicate listeners
    return function cleanupInputHandlers() {
...
/**
 * Send Ctrl+Alt+Del synthetic key combination to remote Windows
 */
export function sendCtrlAltDel(session, { DeviceEvent, InputTransaction }) {
    if (!session) return;
    try {
        const ctrlDown = DeviceEvent.keyPressed(0x1D); // Left Ctrl
        const altDown = DeviceEvent.keyPressed(0x38);  // Left Alt
        const delDown = DeviceEvent.keyPressed(0xE053); // Delete (extended)

        const txDown = new InputTransaction();
        txDown.addEvent(ctrlDown);
        txDown.addEvent(altDown);
        txDown.addEvent(delDown);
        session.applyInputs(txDown);

        setTimeout(() => {
            try {
                const delUp = DeviceEvent.keyReleased(0xE053);
                const altUp = DeviceEvent.keyReleased(0x38);
                const ctrlUp = DeviceEvent.keyReleased(0x1D);

                const txUp = new InputTransaction();
                txUp.addEvent(delUp);
                txUp.addEvent(altUp);
                txUp.addEvent(ctrlUp);
                session.applyInputs(txUp);
            } catch (e) {
                console.error('Failed to release Ctrl+Alt+Del:', e);
            }
        }, 80);
    } catch (err) {
        console.error('Failed to send Ctrl+Alt+Del:', err);
    }
}
