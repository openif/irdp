...
// ────────────────────────────────────────────────────
// Security: Anti-SSRF & Target Validation
// ────────────────────────────────────────────────────

/**
 * Check whether an IP string is a loopback, private, link-local, or reserved address.
 * Covers:
 ...
 * - 0.0.0.0/8, 100.64.0.0/10 (CGNAT), 224.0.0.0/4 (Multicast), 240.0.0.0/4 (Reserved)
...
function isPrivateOrLoopbackIP(ip) {
    ...
    if (ip.startsWith('::ffff:')) {
        return isPrivateOrLoopbackIP(ip.slice(7));
    }
    ...
    return true; // Unknown IP format, treat as unsafe
}

/**
 * Validate that target host and port are safe to connect to.
 * Resolves hostnames via DNS to prevent DNS rebinding or private IP bypass.
 */
async function validateTargetAddress(host, port) {
    ...
    if (lower === 'localhost' || lower.endsWith('.localhost') || lower.endsWith('.local') || lower.endsWith('.internal')) {
        throw new Error(`Access to ${host} is prohibited (private domain)`);
    }

    // Allow bypassing restriction if explicitly configured via environment variable
    if (process.env.ALLOW_PRIVATE_TARGETS === 'true') {
        console.warn(`[Security Warning] ALLOW_PRIVATE_TARGETS is enabled; skipping IP validation for ${host}`);
        return;
    }
    ...
    if (net.isIP(host)) {
        if (isPrivateOrLoopbackIP(host)) {
            throw new Error(`Access to private/loopback IP ${host} is prohibited`);
        }
        return;
    }

    // Hostname: resolve DNS records to check resolved IPs
    try {
        ...
        for (const record of results) {
            if (isPrivateOrLoopbackIP(record.address)) {
                throw new Error(`Host ${host} resolved to prohibited private IP: ${record.address}`);
            }
        }
    } catch (err) {
        if (err.message.includes('prohibited')) throw err;
        throw new Error(`Cannot resolve target host ${host}: ${err.message}`);
    }
}
...
function handleConnection(ws) {
    ...
    ws.once('message', async (data) => {
        try {
            ...
            // Anti-SSRF: Prevent connections to loopback/private/metadata IP addresses
            await validateTargetAddress(host, port);
            ...
            try {
                const httpStatus = err.message.includes('prohibited') ? 403 : 502;
                const errorPdu = buildRDCleanPathError(1, httpStatus);
                ws.send(errorPdu);
            } catch (_) {}
